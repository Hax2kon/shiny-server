...
shinyServer(function(input, output) {
  ...
  nrow(today)/length(unique(today$Spiller))
  seq(1, nrow(today), length(unique(today$Spiller)))
  ...
  today <- today[order(today$Kamptidspukt, today$Spiller),]
  ...
  output$upcoming <- DT::renderDataTable({
    DT::datatable(header(), colnames = c(rep("", ncol(header()))), rownames = rep("", nrow(header())),
